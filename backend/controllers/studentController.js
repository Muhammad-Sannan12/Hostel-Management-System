...
const registerStudent = asyncHandler(async (req, res) => {
    ...
    const {
        ...
        occupation,         // ✅ Added (new schema field)
        fee,                // ✅ Added (new schema field)
    } = req.body;

    
    // Validate required fields (removed program, department, year)
    if (!name || !fatherName || !boardingNumber || !password || !contact || !parentContact) {
        res.status(400);
        throw new Error('Please add all fields');
    }
    ...
    const student = await Student.create({
        ...
        occupation: occupation || undefined,               // ✅ new field
...
const updateStudent = asyncHandler(async (req, res) => {
    ...
    student.contact           = contact           || student.contact;
    student.parentContact     = parentContact     || student.parentContact;
    student.organizationName  = organizationName  !== undefined ? organizationName  : student.organizationName;  // ✅
    student.occupation        = occupation        !== undefined ? occupation        : student.occupation;         // ✅
    ...
    if (roomNumber && hostelName) {
        console.log('🏨 Room update via hostelName+roomNumber:', { hostelName, roomNumber });
            ...
            if (room && room._id.toString() !== (student.room?.toString() || '')) {
                ...
                if (student.room) {
                    ...
                    if (oldRoom) {
                        ...
                        await oldRoom.save();
                        console.log('✅ Old room updated - occupants now:', oldRoom.currentOccupants);
                    }
                }
                ...
                await room.save();
                console.log('✅ New room updated - occupants now:', room.currentOccupants);
            }
        } else {
    ...
    // ── Room assignment via direct room ID ──────────────────────
        ...
        if (newRoomId === null) {
            ...
            if (currentRoomIdStr && currentRoomIdStr !== newRoomIdStr) {
                console.log('❌ Student already in a different room:', currentRoomIdStr);
                res.status(400);
                throw new Error('Student is already assigned to a room. Please unassign them first before assigning a new room.');
            }
...
const deleteStudent = asyncHandler(async (req, res) => {
    ...
    if (student.room) {
        ...
        if (room) {
            ...
            room.occupants = room.occupants.filter(occId => occId.toString() !== student._id.toString());
            if (room.currentOccupants < room.capacity) {
